...
void Adafruit_BNO055::setI2CPort(int port)
{
   _file = port;
}
...
bool Adafruit_BNO055::begin(adafruit_bno055_opmode_t mode)
{
  ...
  {
    usleep(10000); // hold on
  }

  usleep(50000); // hold on
...
*/
/**************************************************************************/
void Adafruit_BNO055::getCalibrationStatus(uint8_t* sys, uint8_t* gyro, uint8_t* accel, uint8_t* mag)
{
  uint8_t calData = read8(BNO055_CALIB_STAT_ADDR);
  if (sys != NULL) {
    *sys = (calData & 0xC0) >> 6;
...
void Adafruit_BNO055::setSensorOffsets(const uint8_t* calibData)
{
    ...
    this->writeLen(ACCEL_OFFSET_X_LSB_ADDR, calibData, NUM_BNO055_OFFSET_REGISTERS);
...
bool Adafruit_BNO055::readLen(adafruit_bno055_reg_t reg, uint8_t * buffer, uint8_t len)
{
  ...
  wBuffer[0] = reg;
  write(_file,wBuffer,1);
  
  if (read(_file, rBuffer, len) != len) {
    return false; 
  }
  else{
    for(int i=0; i<len; i++){
...
bool Adafruit_BNO055::writeLen(adafruit_bno055_reg_t reg, const uint8_t *buffer, uint8_t len)
{
  ...
  uint8_t wBuffer[length];

  memset(wBuffer,'\0', length);

  wBuffer[0] = reg;
  for(int i=1; i < length; i++)
    wBuffer[i] = buffer[i-1];

  if(write(_file,wBuffer,length) != length){
        return false;
    }else{
        return true;
    }
...
/*
//bool Adafruit_BNO055::writeLen(adafruit_bno055_reg_t reg, uint8_t * buffer, uint8_t len)
bool Adafruit_BNO055::writeLen()
{
  uint8_t address = ACCEL_OFFSET_X_LSB_ADDR;
  int count = 0;

  while(count < NUM_BNO055_OFFSET_REGISTERS)
  {
    uint8_t wBuffer[2];
    memset(wBuffer,'\0', 2);
    //uint8_t rBuffer[len];  
